...
const CARD_TYPES = [
  { id: 'pvc', label: 'Karta PVC', price: 89, dims: '85.6 × 54 mm · grubość 0.76 mm', desc: 'Format karty kredytowej. Twarda, wodoodporna, mieści się w każdym portfelu.', accent: '#b44dff' },
...
export default function Home() {
  ...
  const applyDiscount = () => {
    ...
    if (DISCOUNT_CODES[code]) {
      ...
      setDiscountPct(0); setDiscountApplied(false); setDiscountMsg('Nieprawidłowy kod rabatowy.')
    }
  }
  ...
  const handleSubmit = async () => {
    if (!form.name || !form.email || !form.address) { setError('Uzupełnij imię, email i adres wysyłki.'); return }
    if (!agreed) { setError('Zaakceptuj regulamin i politykę prywatności aby kontynuować.'); return }
        ...
        <div className={styles.formBox}>
            ...
            <div className={styles.formStep}>
              ...
              <div className={styles.themesGrid}>
                {FRONT_THEMES.map(t => (
                  <div key={t.id}
                    ...
                    onClick={() => setFrontTheme(t.id)} role="button" tabIndex={0}
                    onKeyDown={e => e.key === 'Enter' && setFrontTheme(t.id)} aria-pressed={frontTheme === t.id}>
                    <div className={styles.themeAccentBar} />
              ...
              <div style={{ background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 'var(--radius-lg)', padding: '20px', marginTop: '12px' }}>
                ...
                <div className={styles.fieldGrid}>
                  <div className={styles.field}><label className={styles.label}>① Rok / wartość</label><input value={form.cardYear} onChange={e => setForm({...form, cardYear: e.target.value})} placeholder="np. 2025 · SEASON 1" /></div>
                  <div className={styles.field}><label className={styles.label}>② Rzadkość</label><input value={form.cardRarity} onChange={e => setForm({...form, cardRarity: e.target.value})} placeholder="COMMON · RARE · EPIC · LEGENDARY" /></div>
                  <div className={`${styles.field} ${styles.fieldFull}`}><label className={styles.label}>③ Nazwa karty</label><input value={form.cardName} onChange={e => setForm({...form, cardName: e.target.value})} placeholder="np. BARON VON KOCH · RAVE FAMILY · ADE 2025" /></div>
                  <div className={styles.field}><label className={styles.label}>④ Atrybut 1 — nazwa</label><input value={form.attr1Label} onChange={e => setForm({...form, attr1Label: e.target.value})} placeholder="np. ENERGY · VIBE · BPM" /></div>
                  <div className={styles.field}><label className={styles.label}>④ Atrybut 1 — wartość</label><input value={form.attr1Value} onChange={e => setForm({...form, attr1Value: e.target.value})} placeholder="np. x3 · x5 · 140" /></div>
                  <div className={`${styles.field} ${styles.fieldFull}`}><label className={styles.label}>⑤ Umiejętność</label><input value={form.cardSkill} onChange={e => setForm({...form, cardSkill: e.target.value})} placeholder="np. HARD TECHNO MASTER · FRIENDS VIBE · PURE FUN" /></div>
                  <div className={styles.field}><label className={styles.label}>⑥ Atrybut 2 — nazwa</label><input value={form.attr2Label} onChange={e => setForm({...form, attr2Label: e.target.value})} placeholder="np. HAPPINESS · SUCCESS RATE" /></div>
                  <div className={styles.field}><label className={styles.label}>⑥ Atrybut 2 — wartość</label><input value={form.attr2Value} onChange={e => setForm({...form, attr2Value: e.target.value})} placeholder="np. x2 · x4" /></div>
                </div>
              </div>
          ...
          {step === 4 && (
            <div className={styles.formStep}>
              ...
              {hasDiscount && <div className={styles.discountBadge}>🎉 Rabat 50% aktywny — zamawiasz {quantity} sztuki tego samego typu!</div>}
              {quantity === 2 && <div className={styles.discountHint}>Dodaj jeszcze 1 kartę i oszczędź 50% na całości!</div>}
              <div className={styles.priceSummary}>
                ...
                {hasDiscount && <p className={styles.summaryRow}><span>Rabat 50%</span><strong className={styles.discount}>−{savedAmount} zł</strong></p>}
                <div className={styles.summaryTotal}><span>Łącznie</span><strong className={styles.totalPrice}>{totalPrice} zł</strong></div>
          ...
          {step === 5 && (
            <div className={styles.formStep}>
              ...
              <div className={styles.fieldGrid}>
                <div className={styles.field}><label className={styles.label}>Imię i nazwisko *</label><input value={form.name} onChange={e => setForm({...form, name: e.target.value})} placeholder="Anna Kowalska" /></div>
                <div className={styles.field}><label className={styles.label}>Email *</label><input type="email" value={form.email} onChange={e => setForm({...form, email: e.target.value})} placeholder="anna@email.com" /></div>
                <div className={styles.field}><label className={styles.label}>Telefon</label><input type="tel" value={form.phone} onChange={e => setForm({...form, phone: e.target.value})} placeholder="+48 500 000 000" /></div>
                <div className={`${styles.field} ${styles.fieldFull}`}><label className={styles.label}>Adres wysyłki *</label><input value={form.address} onChange={e => setForm({...form, address: e.target.value})} placeholder="ul. Przykładowa 1, 00-000 Warszawa" /></div>
              </div>
              ...
              <div style={{ background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 'var(--radius-lg)', padding: '16px', marginTop: '8px' }}>
                ...
                <div style={{ display: 'flex', gap: '8px' }}>
                  <input value={discountCode} onChange={e => { setDiscountCode(e.target.value.toUpperCase()); setDiscountMsg(null); setDiscountApplied(false); setDiscountPct(0) }} placeholder="np. RAVE10" style={{ flex: 1, textTransform: 'uppercase', letterSpacing: '2px' }} disabled={discountApplied} />
                  <button onClick={applyDiscount} disabled={!discountCode.trim() || discountApplied} className={styles.btnSecondary} style={{ width: 'auto', padding: '10px 18px', fontSize: '13px', flexShrink: 0 }}>{discountApplied ? '✓ Aktywny' : 'Zastosuj'}</button>
                </div>
              ...
              <div className={styles.priceSummary} style={{ marginTop: '8px' }}>
                ...
                <p className={styles.summaryRow}><span>Cena za sztukę</span><strong>{unitPrice} zł</strong></p>
                <p className={styles.summaryRow}><span>Ilość</span><strong>× {quantity}</strong></p>
                {hasDiscount && <p className={styles.summaryRow}><span>Rabat ilościowy (50%)</span><strong className={styles.discount}>−{savedAmount} zł</strong></p>}
                {discountApplied && <p className={styles.summaryRow}><span>Kod rabatowy ({discountCode.toUpperCase()} −{discountPct}%)</span><strong className={styles.discount}>−{discountSaved} zł</strong></p>}
                <div className={styles.summaryTotal}><span>Do zapłaty</span><strong className={styles.totalPrice}>{totalPrice} zł</strong></div>
