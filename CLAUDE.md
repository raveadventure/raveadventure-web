...
- **[✅ KONTO POTWIERDZONE 2026-08-05, integracja jeszcze niewdrożona] Automatyczne płatności online — wdrażamy Paybylink.** Wcześniej świadomie brak automatycznych płatności; Michał założył osobne, dedykowane konto bankowe firmowe dla RaveAdventure (płatności BLIK już przekierowane na nowe konto) i **ma już potwierdzone konto Paybylink**. **Korekta ważnej pomyłki z wcześniejszej sesji**: CLAUDE.md błędnie zakładał, że "PayByLink" = Blue Media — to nieprawda. Faktyczny dostawca to **Paybylink (paybylink.pl)**, usługa świadczona przez **Systemy Płatnicze Robert Paszkowski**, ul. Jana Pawła II 22, 00-133 Warszawa, NIP 1182105129, REGON 360726494, wpisana do rejestru małych instytucji płatniczych (MIP) prowadzonego przez KNF pod numerem MIP31/2019. Obsługuje BLIK oraz Przelewy Online (szybkie przelewy), prowizja 1.49% + brak opłaty stałej (poniżej 0.30 zł minimum), reklamacje płatności → platnosci@rpdp.pl (rozdzielone od reklamacji produktu → RaveAdventure). **Nie proponuj Stripe/PayU/Blue Media** — to świadomie odrzucone lub błędne alternatywy. **2026-08-05: zaktualizowany `app/regulamin/page.tsx`** (dodany adres do korespondencji Michała, ujawnienie dostawcy płatności Paybylink w sekcji "Ceny i płatności", nowa sekcja "Prawo odstąpienia od umowy" — wyłączenie 14-dniowego prawa zwrotu dla produktów personalizowanych zgodnie z art. 38 pkt 3 ustawy o prawach konsumenta, dotąd brakujące) — to było świadomie zrobione PRZED samą integracją API, na wyraźną prośbę Michała ("trzeba w regulaminie dodać informacje o płatnościach zanim dodamy"). **Sama integracja API (kod płatności) jeszcze nie zaczęta** — do zrobienia, gdy Michał dostarczy dokumentację API/dane dostępowe do Panelu Paybylink. Do czasu integracji płatność działa jak dotąd: BLIK/przelew ręczny, dane wysyłane w mailu z projektem karty (`/api/send-design`, stałe `BLIK_PHONE`/`BANK_ACCOUNT`/`BANK_RECIPIENT` na górze pliku).
...
- Jeśli od ostatniej sesji minęło więcej niż kilka dni, dopytaj o status integracji Paybylink (patrz „Niezmienniki biznesowe i bezpieczeństwo" wyżej) — konto potwierdzone 2026-08-05, regulamin zaktualizowany, ale sama integracja API czeka na dokumentację/dane dostępowe od Michała z Panelu Paybylink.
